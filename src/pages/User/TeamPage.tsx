...
const TeamPage = () => {
  const { tournamentId, teamName } = useParams<{
    tournamentId: string;
    teamName: string;
  }>();
    ...
    const fetchTournament = async () => {
      if (tournamentId != null) {
        ...
        if (docSnap.exists()) {
          ...
          if (tournament.phases[tournament.currentPhase]?.type === "Poules")
            setCurrentGroup(
              tournament.phases[tournament.currentPhase].groups!.find((g) =>
                g.teams!.includes(teamName!),
              ) ?? null,
            );
        } else {
    ...
    if (tournament) {
      ...
      if (currentPhase) {
        if (currentPhase.type === PhaseType.GROUP) {
          matches =
            currentPhase.groups?.flatMap((group) => group.matches || []) || [];
        } else {
      ...
      const teamMatches =
        matches?.filter((match) => match.teams.includes(teamName!)) || [];
  ...
  return (
    <div className="min-h-screen bg-[#F9F9FB]">
      {currentPhase && (
        <>
          <div className=" mb-4 flex items-center justify-center gap-4 px-10 py-6">
            <img
          ...
          <div className="px-4">
            {currentPhase.type === "Poules" ? (
              ...
              <>
                ...
                {teamMatches.map((m) => (
                  <div className="border-primary m-4 rounded-xl border-2 p-4">
